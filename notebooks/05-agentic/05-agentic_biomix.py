    ...
    while True:
        ...
        if len(code_blocks) == 0:
            ...
            if current_status == 'help':
                ...
                response = "Unfortunately I don't know" #input("Your answer:")
...
for i,question in enumerate(questions):
    ...
    template_name = f"biomix-q{i}-claude-1"
    output_files = os.path.join(scratch_dir, f"{template_name}.txt")
    if os.path.exists(output_files):
        print(f"Skipping {template_name} as it already exists")
        continue
    else:
        ...
        try:
            ...
            notification("chord.wav")
            print(f"Error: {e}")

notification("tada.wav")
